...
export function TasksCardList({ columnId }: { columnId: number }) {
  ...
  const cardTasks = filteredTasks.map((item, index) => (
        ...
        >
          <Paper key={item.id} w={310} shadow="sm" radius="md" p="xs" withBorder>
            ...
            <Group justify="space-between">
              <ActionIcon variant="transparent" color="gray" aria-label="Settings" size="md">
                <IconMessageCircle />
                {"3"}
              </ActionIcon>
  ...
  return (
    <Droppable droppableId={String(columnId)} direction="vertical">
      {(provided) => (
